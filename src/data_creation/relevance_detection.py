            ...
            if pd.isna(ex[f"{aspect}_label"]):
                ...
                for idx, sentence in enumerate(sentences):
                    ...
                    if sentence[3:].lstrip().startswith(f"{tag}"):
                        ...
                        if not response:
                            response = f"{idx + 1}. [Relevant]"  # Reasons: {relevance_reason}"
                        else:
                            response = f"{response}\n{idx + 1}. [Relevant]"  # Reasons: {relevance_reason}"
                ...
                if len(response.split("\n")) == len(merged_answer_units.split("\n")):
                    ...
                    # df['identified_answer'] = df['identified_answer'].astype(str)


            if not response.__contains__("Irrelevant"):
                # print(response)
                rel_count += 1
                # print("---------------------------")
                # print(output)
                ...
                #     print(sent[3:].lstrip())
                #     print(sent[2:].lstrip().startswith(f"{tag}"))
                # print(response)
                print("============================")
        print(rel_count)
...
if __name__ == '__main__':
    ...
    aspect = "irrelevance"  # "irrelevance", "incomplete_ans", "reference_example"
    use_all_data = False
    use_reason = True
    add_score = False
    ...
    for i, row in complete_df.iterrows():
        ...
        # print("answer: ", row["identified_answer"])
        examples.append({
    ...
    utils.jdump(examples, f"src/data/annotated_data/{aspect}_detection_data.jsonl")
